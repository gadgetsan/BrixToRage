...
export default class StorageLocInfoModal extends Component {
    constructor(props) {
        super(props);
    ...
    fetchData() {
        helpers.fetchHelperWithData("getPart", { rebrickableId: this.props.part.RebrickableId }, partData => {
    ...
    fetchColors() {
        helpers.fetchHelperWithData("getColorsForPart", { rebrickableId: this.props.part.RebrickableId }, colorData => {
    ...
    fetchColorInfo(colorId) {
        ...
        helpers.fetchHelperWithData("fetchColor", { rebrickableId: colorId }, colorData => {
            this.fetchColors();
        });
    }
        ...
        return (
            <>
                ...
                <MDBModal isOpen={this.state.modal} toggle={this.toggle(4)} size="lg">
                    <MDBModalHeader toggle={this.toggle(4)}>Information for {this.state.loading ? "..." : this.state.partData.Name}</MDBModalHeader>
                    <MDBModalBody style={{ textAlign: "left" }}>
                        ...
                        <strong>Name: </strong> {this.state.loading ? "..." : this.state.partData.Name}
                        <br />
                        ...
                        <strong>
                            ...
                            {this.state.loadingColors ? (
                                ...
                                this.state.colorData.map(color => <PartListModalColorInfo key={color.Id} color={color} fetchColorInfo={this.fetchColorInfo} />)
                            )}
                        </strong>{" "}
                        <br />
